...
export class DatabaseAgent {
    constructor(firestoreService, schemaValidator, performanceMonitor) {
    ...
    async createUser(userData) {
        // 스키마 검증
        const validation = this.schemaValidator.validate('User', userData);
        if (!validation.valid) {
            throw new Error(`사용자 데이터 검증 실패: ${validation.errors?.join(', ')}`);
        }
        // 성능 측정과 함께 사용자 생성
        const { result } = await this.performanceMonitor.measureWrite('createUser', COLLECTIONS.USERS, async () => {
            // 중복 이메일 확인
            const existingUsers = await this.firestoreService.query(COLLECTIONS.USERS, (ref) => ref.where('core.email', '==', userData.core.email).limit(1));
            if (existingUsers.length > 0) {
                throw new Error('이미 존재하는 이메일입니다.');
            }
            // 사용자 생성
            return await this.firestoreService.create(COLLECTIONS.USERS, validation.data, userData.core.uid);
        });
     ...
     * 감정 체크인 저장
     */
    async saveEmotionCheckin(checkinData) {
        // 스키마 검증
        const validation = this.schemaValidator.validate('EmotionCheckin', checkinData);
        if (!validation.valid) {
        ...
        // 성능 측정과 함께 저장
        const { result } = await this.performanceMonitor.measureWrite('saveEmotionCheckin', COLLECTIONS.EMOTIONS, async () => {
            return await this.firestoreService.create(COLLECTIONS.EMOTIONS, validation.data, checkinData.id);
        });
        // 감정 트렌드 업데이트 (비동기)
    ...
    async saveJournalEntry(journalData) {
        // 스키마 검증
        const validation = this.schemaValidator.validate('JournalEntry', journalData);
        if (!validation.valid) {
            throw new Error(`일기 데이터 검증 실패: ${validation.errors?.join(', ')}`);
        }
        // 성능 측정과 함께 저장
        const { result } = await this.performanceMonitor.measureWrite('saveJournalEntry', COLLECTIONS.JOURNALS, async () => {
            return await this.firestoreService.create(COLLECTIONS.JOURNALS, validation.data, journalData.id);
        });
     ...
     * 전문가 프로필 저장
     */
    async saveExpertProfile(expertData) {
        // 스키마 검증
        const validation = this.schemaValidator.validate('ExpertProfile', expertData);
        if (!validation.valid) {
            throw new Error(`전문가 프로필 검증 실패: ${validation.errors?.join(', ')}`);
        }
        // 성능 측정과 함께 저장
        const { result } = await this.performanceMonitor.measureWrite('saveExpertProfile', COLLECTIONS.EXPERTS, async () => {
            return await this.firestoreService.create(COLLECTIONS.EXPERTS, validation.data, expertData.uid);
        });
     ...
     * 사용자별 감정 데이터 조회
     */
    async getUserEmotions(userId, limit = 50, startDate, endDate) {
        const { result } = await this.performanceMonitor.measureQuery('getUserEmotions', COLLECTIONS.EMOTIONS, async () => {
            return await this.firestoreService.query(COLLECTIONS.EMOTIONS, (ref) => {
     ...
     * 위기 상황 감지 및 대응
     */
    async handleCrisisDetection(checkinData) {
     ...
     * 감정 트렌드 업데이트
     */
    async updateEmotionTrends(userId) {
        try {
            // 최근 7일간의 감정 데이터 조회
            ...
            const avgIntensity = recentEmotions.reduce((sum, e) => sum + e.structure.dominantEmotion.intensity, 0) / recentEmotions.length;
            // 트렌드 방향 계산
     ...
     * 안정성 점수 계산
     */
    calculateStabilityScore(emotions) {
     ...
     * 트렌드 인사이트 생성
     */
    generateTrendInsights(emotions, direction) {
        const insights = [];
        // 방향별 인사이트
        switch (direction) {
            case 'improving':
    ...
    async runIntegrityCheck() {
        ...
        try {
            // 샘플 데이터 검증
            const sampleUsers = await this.firestoreService.query(COLLECTIONS.USERS, (ref) => ref.limit(5));
            for (const user of sampleUsers) {
                const validation = this.schemaValidator.validate('User', user);
                if (!validation.valid) {
