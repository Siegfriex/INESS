...
export class SchemaValidator {
    ...
    initializeSchemas() {
        ...
        this.schemas.set('EmotionCheckin', Joi.object({
            ...
            structure: Joi.object({
                emotions: Joi.array().items(Joi.object({
                    primary: Joi.string().valid('joy', 'sadness', 'anger', 'fear', 'surprise', 'disgust', 'trust', 'anticipation').required(),
        ...
        this.schemas.set('JournalEntry', Joi.object({
            ...
            content: Joi.object({
                ...
                category: Joi.string().valid('daily', 'gratitude', 'reflection', 'goal_setting', 'crisis', 'celebration', 'other').optional(),
        ...
        this.schemas.set('ExpertProfile', Joi.object({
            ...
            credentials: Joi.object({
                ...
                verificationStatus: Joi.string().valid('pending', 'verified', 'rejected', 'expired').required(),
            ...
            specialties: Joi.object({
                ...
                approaches: Joi.array().items(Joi.string().valid('CBT', 'DBT', 'mindfulness', 'psychodynamic', 'humanistic', 'family_therapy', 'other')).min(1).required(),
                languages: Joi.array().items(Joi.string()).min(1).required(),
                ageGroups: Joi.array().items(Joi.string().valid('children', 'adolescents', 'adults', 'seniors')).min(1).required(),
    ...
    validatePartial(schemaName, data) {
        ...
        const partialSchema = schema.fork(Object.keys(schema.describe().keys), (schema) => schema.optional());
     ...
     * 커스텀 검증 규칙 추가
     */
    addCustomValidation(schemaName, fieldPath, validationFn) {
        const schema = this.schemas.get(schemaName);
        if (!schema) {
     ...
     * 필드별 검증
     */
    validateField(schemaName, fieldName, value) {
        const schema = this.schemas.get(schemaName);
        if (!schema) {
            return {
                valid: false,
     ...
     * 스키마 설명에서 필드 스키마 추출
     */
    getFieldSchema(schemaDescription, fieldPath) {
