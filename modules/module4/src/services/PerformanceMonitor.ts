...
export class PerformanceMonitor {
   ...
   * 컬렉션별 메트릭 수집
   */
  private async collectCollectionMetrics(collectionName: string): Promise<void> {
    const startTime = Date.now();
    
    try {
      // 컬렉션 문서 수 확인 (샘플링)
      const sampleQuery = () => this.firestoreService.query(
        collectionName,
        (ref) => ref.limit(1)
      );

      const { result, metrics } = await this.measureQuery(
        'health_check',
        collectionName,
        sampleQuery
      );
  ...
  private async analyzePerformanceTrends(): Promise<void> {
    for (const [collection, metricsList] of this.metrics) {
      if (metricsList.length < 5) continue; // 충분한 데이터가 없으면 스킵
   ...
   * 중요 성능 이슈 알림
   */
  private async alertCriticalPerformance(
    collection: string,
   ...
   * 메트릭 기록
   */
  private async recordMetrics(collection: string, metrics: PerformanceMetrics): Promise<void> {
   ...
   * 현재 읽기 카운트 (모의 구현)
   */
  private async getCurrentReadCount(): Promise<number> {
   ...
   * 현재 쓰기 카운트 (모의 구현)  
   */
  private async getCurrentWriteCount(): Promise<number> {
   ...
   * 작업별 통계
   */
  private getOperationStats(metrics: PerformanceMetrics[]): Record<string, any> {
    const operations: Record<string, { count: number; avgTime: number }> = {};
